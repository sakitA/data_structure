...
class Logo:
    def __init__(self, scene):
        svg = SVGMobject("assets/logo.svg").scale(3)
        ...
        text_group = VGroup(text_o, text_top, text_bottom)
        text_group.next_to(svg, direction=RIGHT)

        _ = VGroup(svg, text_group).move_to(ORIGIN)

        animations = []
        for obj in svg.submobjects:
            animations.append(Create(obj, run_time=1))
        for text in text_group.submobjects:
            animations.append(Write(text))

        scene.play(*animations, run_time=2)
        scene.wait(0.5)
